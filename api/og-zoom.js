...
async function ensureWasm() {
  ...
  const wasmPath = (0, import_node_path.join)(process.cwd(), "node_modules/@resvg/resvg-wasm/index_bg.wasm");
  const wasmBuf = (0, import_node_fs.readFileSync)(wasmPath);
  await (0, import_resvg_wasm.initWasm)(wasmBuf);
...
async function handler(req, res) {
  ...
  <text x="1120" y="585" font-family="-apple-system, system-ui, sans-serif" font-weight="500"
    ...
    if (format === "png") {
      await ensureWasm();
      const resvg = new import_resvg_wasm.Resvg(svg, { fitTo: { mode: "width", value: 1200 } });
