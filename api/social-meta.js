...
// server/_vercel/social-meta.ts
var social_meta_exports = {};
__export(social_meta_exports, {
  default: () => handler
});
module.exports = __toCommonJS(social_meta_exports);
...
// server/_vercel/social-meta.ts
var SITE_ORIGIN = "https://turboloop.tech";
var DEFAULT_IMAGE = `${SITE_ORIGIN}/api/og-banner?type=launch`;
var LOGO = "https://pub-1d13f4e7ccfa4575bc04b75045f1b1b1.r2.dev/branding/turboloop-logo.png";
var _db = null;
function getDb() {
  if (_db) return _db;
...
function thumbForReelUrl(videoUrl) {
  try {
...
function slugFromReelUrl(videoUrl) {
  try {
    const u = new URL(videoUrl);
  ...
  try {
    ...
    if (blogMatch) {
      ...
      if (post) {
        meta = {
          ...
          image: post.coverImage || `${SITE_ORIGIN}/api/og?slug=${encodeURIComponent(slug)}`,
          type: "article",
          publishedTime: post.createdAt ? new Date(post.createdAt).toISOString() : void 0
        };
        return meta;
      }
    }
    ...
    if (reelMatch) {
      ...
      const reel = allVideos.find(
        (v2) => v2.directUrl && slugFromReelUrl(v2.directUrl) === slug
      );
    ...
    if (filmMatch) {
      const slug = decodeURIComponent(filmMatch[1]).replace(/-/g, " ").replace(/\b\w/g, (c) => c.toUpperCase());
      meta = {
        title: trim(`${slug} \u2014 TurboLoop Cinematic Universe`, 90),
        description: "Watch this short film from the TurboLoop Cinematic Universe \u2014 20 films across 4 seasons.",
        image: DEFAULT_IMAGE,
        type: "video.other"
      };
      return meta;
    ...
    if (topicMatch) {
      const tag = decodeURIComponent(topicMatch[1]).replace(/-/g, " ").replace(/\b\w/g, (c) => c.toUpperCase());
      meta = {
    ...
    if (vsMatch) {
      const slug = decodeURIComponent(vsMatch[1]).replace(/-/g, " ").replace(/\b\w/g, (c) => c.toUpperCase());
      meta = {
    ...
    if (learnMatch) {
      const slug = decodeURIComponent(learnMatch[1]).replace(/-/g, " ").replace(/\b\w/g, (c) => c.toUpperCase());
      meta = {
    ...
    if (ecoMatch) {
      const slug = decodeURIComponent(ecoMatch[1]).replace(/-/g, " ").replace(/\b\w/g, (c) => c.toUpperCase());
      meta = {
...
function buildHtml(path, meta) {
  ...
  const publishedTime = meta.publishedTime ? `<meta property="article:published_time" content="${escapeHtml(meta.publishedTime)}" />` : "";
  return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
...
async function handler(req, res) {
  try {
    ...
    const fallback = buildHtml("/", {
      title: "Turbo Loop \u2014 Sustainable DeFi Yield on BSC",
      description: "Audited, renounced, 100% LP-locked yield farming on Binance Smart Chain.",
      image: DEFAULT_IMAGE,
      type: "website"
    });
    res.statusCode = 200;
