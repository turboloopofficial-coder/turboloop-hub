...
function detectCallout(children: ReactNode): { kind: CalloutKind; rest: ReactNode } | null {
  ...
  const innerArr = Array.isArray(first.props.children) ? first.props.children : [first.props.children];
...
const CALLOUT_STYLES: Record<CalloutKind, { bg: string; border: string; text: string; icon: typeof Info; label: string }> = {
  info: { bg: "rgba(8,145,178,0.06)", border: "rgba(8,145,178,0.25)", text: "#0E7490", icon: Info, label: "Info" },
  tip: { bg: "rgba(16,185,129,0.06)", border: "rgba(16,185,129,0.25)", text: "#047857", icon: Lightbulb, label: "Tip" },
  warn: { bg: "rgba(245,158,11,0.06)", border: "rgba(245,158,11,0.3)", text: "#92400E", icon: AlertTriangle, label: "Heads-up" },
  key: { bg: "rgba(124,58,237,0.06)", border: "rgba(124,58,237,0.25)", text: "#5B21B6", icon: Sparkles, label: "Key insight" },
};
  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: `linear-gradient(135deg, ${s.text}, ${s.text}dd)` }}
      >
...
export default function BlogContent({ content, palette, slug = "" }: Props) {
  ...
  const components = {
    ...
    p({ children, ...rest }: any) {
      return (
        <p className="text-base md:text-[17px] text-slate-700 leading-[1.8] my-5" {...rest}>
          {children}
        </p>
      );
    },
    blockquote({ children, ...rest }: any) {
      ...
      return (
        <blockquote
          className="my-10 relative pl-8 pr-4 py-2"
          {...rest}
        >
          <div
            className="absolute left-0 top-0 bottom-0 w-1 rounded-full"
            style={{ background: `linear-gradient(180deg, ${palette.from}, ${palette.via}, ${palette.to})` }}
          />
          <Quote
    ...
    ul({ children, ...rest }: any) {
      return <ul className="my-5 pl-6 space-y-2 list-disc marker:text-slate-400" {...rest}>{children}</ul>;
    },
    ol({ children, ...rest }: any) {
      return <ol className="my-5 pl-6 space-y-2 list-decimal marker:text-slate-400 marker:font-bold" {...rest}>{children}</ol>;
    },
    li({ children, ...rest }: any) {
      return <li className="text-base md:text-[17px] text-slate-700 leading-[1.7] pl-2" {...rest}>{children}</li>;
    },
    a({ children, href, ...rest }: any) {
      ...
      return (
        <a
          ...
          style={{ color: palette.from, textDecorationColor: `${palette.from}55` }}
          {...rest}
        >
          {children}
        </a>
    ...
    strong({ children, ...rest }: any) {
      return <strong className="font-bold text-slate-900" {...rest}>{children}</strong>;
    },
    ...
    hr() {
      return (
        <div className="my-12 flex items-center justify-center gap-3">
          <div className="h-px flex-1 max-w-32" style={{ background: `linear-gradient(90deg, transparent, ${palette.from}40, transparent)` }} />
          <div
            className="w-2 h-2 rounded-full"
            style={{ background: palette.via, boxShadow: `0 0 12px ${palette.via}` }}
          />
          <div className="h-px flex-1 max-w-32" style={{ background: `linear-gradient(90deg, transparent, ${palette.from}40, transparent)` }} />
        </div>
      );
    ...
    table({ children, ...rest }: any) {
      return (
        <div className="my-8 overflow-x-auto rounded-xl" style={{ border: "1px solid rgba(15,23,42,0.08)" }}>
    ...
    th({ children, ...rest }: any) {
      return (
        <th
          ...
          style={{ background: `${palette.from}10`, borderBottom: `1px solid ${palette.from}25` }}
          {...rest}
        >
          {children}
        </th>
    ...
    td({ children, ...rest }: any) {
      return <td className="px-4 py-3 text-slate-700 border-t border-slate-100" {...rest}>{children}</td>;
    },
  };
