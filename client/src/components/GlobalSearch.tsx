...
export default function GlobalSearch({ open, onClose }: Props) {
  ...
  const { data: blogPosts } = trpc.content.blogPosts.useQuery(undefined, { enabled: open });
  const { data: videos } = trpc.content.videos.useQuery(undefined, { enabled: open });
  ...
  const fullIndex = useMemo(() => {
    const blogResults: SearchResult[] = (blogPosts ?? []).map((p: any) => ({
      ...
      href: `/blog/${p.slug}`,
      emoji: "📖",
      searchBlob: `${p.title} ${p.excerpt || ""} ${(p.content || "").slice(0, 2000)}`.toLowerCase(),
    }));
    const reelResults: SearchResult[] = (videos ?? [])
      .filter((v: any) => v.directUrl && !v.youtubeUrl && v.category !== "cinematic")
      .map((v: any) => {
  ...
  const results = useMemo(() => searchAll(fullIndex, query, 30), [fullIndex, query]);
    ...
    <AnimatePresence>
      {open && (
        ...
        >
          ...
          >
            {/* Search input */}
            <div className="flex items-center gap-3 px-5 py-4 border-b" style={{ borderColor: "rgba(15,23,42,0.06)" }}>
              ...
              <button onClick={onClose} className="md:hidden p-1 rounded-md hover:bg-slate-100" aria-label="Close search">
            ...
            <div className="max-h-[60vh] overflow-y-auto">
              {!query && (
                <div className="p-8 text-center">
                  ...
                  <p className="text-sm text-slate-500">
                    Type to search across films, blog posts, lessons, ecosystem pillars, comparisons, reels, and creatives.
                  </p>
                  <div className="mt-4 flex flex-wrap items-center justify-center gap-1.5">
                    {["security", "yield", "stablecoin", "ponzi", "renounced", "compounding"].map((s) => (
                      <button
                        key={s}
                        onClick={() => setQuery(s)}
              ...
              {query && results.length === 0 && (
                <div className="p-8 text-center">
                  ...
                  <p className="text-sm text-slate-500">No results for "<span className="font-mono text-slate-700">{query}</span>"</p>
                </div>
              )}
                    ...
                    {group.items.map((r) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <ArrowRight className={`w-4 h-4 shrink-0 transition ${active ? "text-cyan-600 translate-x-0.5" : "text-slate-300"}`} />
                        </button>
                      );
                    })}
                  </div>
            ...
            {/* Footer hint */}
            <div className="px-5 py-3 border-t flex items-center justify-between text-[11px] text-slate-400" style={{ borderColor: "rgba(15,23,42,0.06)" }}>
              <div className="flex items-center gap-3">
                <span className="inline-flex items-center gap-1">
                  <kbd className="px-1.5 py-0.5 rounded font-mono bg-slate-100">↑↓</kbd>
                  navigate
                </span>
                <span className="inline-flex items-center gap-1">
                  <kbd className="px-1.5 py-0.5 rounded font-mono bg-slate-100">↵</kbd>
...
function groupOrderBefore(a: string, b: string): boolean {
  const order = ["page", "film", "blog", "lesson", "pillar", "comparison", "reel", "creative"];
