...
export default function ReelPage() {
  ...
  const reels = useMemo(() => (allVideos ?? []).filter(v => v.directUrl), [allVideos]);
  const reel = useMemo(() => reels.find(v => slugFromUrl(v.directUrl!) === slug), [reels, slug]);
  const others = useMemo(() => reels.filter(v => v.id !== reel?.id).slice(0, 6), [reels, reel]);
  ...
  const jsonLd = reel ? {
    ...
    thumbnailUrl: thumb,
    uploadDate: typeof reel.createdAt === "string" ? reel.createdAt : new Date(reel.createdAt).toISOString(),
  ...
  return (
    <div className="min-h-screen" style={{ background: "linear-gradient(180deg, #0f172a 0%, #1e293b 100%)" }}>
      ...
      {/* Header */}
      <header className="sticky top-0 z-50 border-b border-white/10" style={{ background: "rgba(15,23,42,0.85)", backdropFilter: "blur(20px)" }}>
      ...
      <div className="container py-8 md:py-12">
        ...
        <nav aria-label="Breadcrumb" className="flex items-center gap-1.5 text-xs text-white/40 mb-6">
          ...
          <ChevronRight className="h-3 w-3" />
          <span className="text-white/70 truncate max-w-[200px]">{reel?.title || "Loading…"}</span>
        </nav>
        ...
        {reel ? (
          <div className="grid md:grid-cols-5 gap-8 max-w-5xl mx-auto">
            ...
            <div className="md:col-span-3">
              <div className="relative rounded-2xl overflow-hidden bg-black shadow-2xl" style={{ aspectRatio: "9 / 16", maxHeight: "80vh" }}>
                <video
                  ref={videoRef}
                  src={reel.directUrl!}
                  poster={thumb}
            ...
            <div className="md:col-span-2 text-white">
              <h1 className="text-2xl md:text-3xl font-bold leading-tight mb-3">{reel.title}</h1>
              <p className="text-white/60 text-sm leading-relaxed mb-5">
                A short explainer on why Turbo Loop is the safest, most transparent yield protocol in DeFi.
                Watch more reels and discover the complete ecosystem.
              </p>
              ...
              <div className="mt-8 pt-6 border-t border-white/10">
                <div className="text-[11px] tracking-wider uppercase text-white/40 mb-3">More Reels</div>
                <div className="grid grid-cols-3 gap-2">
                  {others.map((o) => {
                    const oSlug = slugFromUrl(o.directUrl!) || "";
                    return (
                      <Link key={o.id} href={`/reels/${oSlug}`}>
                        <div className="group relative rounded-lg overflow-hidden cursor-pointer" style={{ aspectRatio: "9 / 16" }}>
                          <img src={thumbForReel(o.directUrl!)} alt={o.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
          ...
          <div className="text-center py-20 text-white/60">
            <p>Reel not found.</p>
            <Link href="/"><button className="mt-4 text-cyan-400 hover:text-cyan-300 text-sm font-medium">← Back to home</button></Link>
          </div>
        )}
      </div>
    </div>
