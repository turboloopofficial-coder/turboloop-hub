...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    ...
    return {
      ...
      alternates: {
        canonical: `https://www.turboloop.tech/${locale}/community`,
      },
    };
  } catch {
    return {};
...
export default async function LocaleCommunityPage({ params }: Props) {
  const { locale } = await params;
  if (!hasLocale(routing.locales, locale)) redirect("/community");
  const { default: Page } = await import("../../community/page");
  return <Page />;
}
