...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    ...
    return {
      ...
      alternates: {
        canonical: locale === "en"
        ? "https://www.turboloop.tech/films"
        : `https://www.turboloop.tech/${locale}/films`,
      },
    };
  } catch {
    return {};
...
export default async function LocaleFilmsPage({ params, searchParams }: Props) {
  const { locale } = await params;
  if (!hasLocale(routing.locales, locale)) redirect("/films");
  const { default: Page } = await import("../../films/page");
