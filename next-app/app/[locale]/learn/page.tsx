...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    ...
    return {
      ...
      alternates: {
        canonical: locale === "en"
        ? "https://www.turboloop.tech/learn"
        : `https://www.turboloop.tech/${locale}/learn`,
      },
    };
  } catch {
    return {};
...
export default async function LocaleLearnPage({ params }: Props) {
  const { locale } = await params;
  if (!hasLocale(routing.locales, locale)) redirect("/learn");
  const { default: Page } = await import("../../learn/page");
  return <Page />;
}
